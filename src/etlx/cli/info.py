...
from etlx.engines.backends import BACKENDS, list_backends
...
def info(
    ...
    Examples:
        etlx info                  # Show version info
        etlx info --backends       # List available backends
        etlx info --check          # Check backend availability
        etlx info --config p.yml   # Show pipeline info
    """
    if config_file:
...
def _show_version_info() -> None:
    ...
    console.print("\n[bold]Commands:[/bold]")
    console.print("  etlx run <config>       Run a pipeline")
    console.print("  etlx validate <config>  Validate configuration")
    console.print("  etlx init <name>        Initialize new project")
    console.print("  etlx info               Show this information")
    console.print("  etlx info --backends    List available backends")
...
def _show_backends(check_imports: bool) -> None:
    ...
    if not check_imports:
        console.print("\nUse [cyan]etlx info --check[/cyan] to verify backend availability")
...
def _show_pipeline_info(config_file: Path) -> None:
    ...
    from etlx.config.loader import load_pipeline_config
